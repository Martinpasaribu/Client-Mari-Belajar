...
import { Search, GraduationCap, ArrowRight, Library } from "lucide-react";
...
export default function MyModulesPage() {
  ...
  useEffect(() => {
    const fetchMyModules = async () => {
      try {
        // Kita beri sedikit delay agar animasi loading terasa smooth
        setLoading(true);
        const { data } = await api.get('/enrollments/my-modules');
        
        // Simulasi delay minimal agar user sempat melihat loader premium
  ...
  return (
    <>
      {/* 1. Pasang MainLoader dengan title 'category' atau custom string */}
      <MainLoading isOpen={loading} title="category" />

      <FadeInContainer className="space-y-12 pb-20">
        
        {/* HEADER & STATS SECTION */}
        <FadeInItem className="relative overflow-hidden bg-primary-1 dark:bg-dark-primary-2 rounded-[3rem] p-4 px-6 md:p-6 md:px-10 text-white shadow-2xl shadow-primary-1/10">
          <div className="absolute top-[-20%] right-[-10%] h-64 w-64 rounded-full bg-primary-1/20 blur-[100px]" />
          
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10">
                <Library size={12} className="text-white" />
                <span className="text-[9px] font-black uppercase tracking-widest text-white">Personal Library</span>
              </div>
              <h1 className="text-3xl md:text-4xl font-black tracking-tighter uppercase leading-none">
                Modul <span className="opacity-60 italic">Saya</span>
              </h1>
              <p className="text-white/70 dark:text-primary-1 font-medium max-w-md">
            ...
            <div className="flex gap-4 sm:gap-6">
              <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-[2rem] min-w-[120px] text-center">
                <p className="text-[10px] font-black text-white/50 uppercase mb-1 tracking-widest">Total Modul</p>
                <p className="text-4xl font-black text-white">{modules.length}</p>
              </div>
              <div className="bg-white/10 backdrop-blur-md border border-white/10 p-6 rounded-[2rem] min-w-[120px] text-center">
                <p className="text-[10px] font-black text-white/50 uppercase mb-1 tracking-widest">Aktif</p>
                <p className="text-4xl font-black text-white">
                  {modules.filter(m => m.status === 'success' || m.status === 'active').length}
                </p>
              </div>
            </div>
          </div>
        ...
        <FadeInItem className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4">
          <div className="flex items-center gap-3">
              <div className="h-10 w-1 bg-primary-1 rounded-full" />
          ...
          <div className="relative group w-full md:w-80">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary-1 transition-colors" size={18} />
            <input 
              type="text" 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="CARI JUDUL MODUL..."
              className="w-full pl-12 pr-4 py-4 rounded-2xl border border-slate-200 dark:border-white/5 bg-white dark:bg-slate-900 text-[10px] font-black uppercase tracking-widest focus:outline-none focus:ring-2 focus:ring-primary-1/20 transition-all shadow-sm"
            />
          </div>
        </FadeInItem>
          ...
          filteredModules.length > 0 ? (
            ...
            <FadeInItem className="flex flex-col items-center justify-center py-24 bg-white dark:bg-slate-900/50 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-white/5">
              ...
              <Link 
                ...
                className="flex items-center gap-2 bg-primary-1 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:shadow-xl hover:shadow-primary-1/20 transition-all active:scale-95"
              >
